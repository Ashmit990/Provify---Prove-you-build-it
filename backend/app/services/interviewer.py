...
def generate_question(state: InterviewState) -> InterviewState:
    ...
    prompt = (
        ...
        f"- CRITICAL: only ask about a failure mode, race condition, or edge "
        f"case if it can ACTUALLY occur given how this exact code is written. "
        f"JavaScript is single-threaded — do not imply concurrent/simultaneous "
        f"execution of event handlers unless the code shown genuinely involves "
        f"async callbacks, promises, or timers that could capture stale state. "
        f"If a plain synchronous line of code has no real failure mode, ask "
        f"about something else in the chunk instead of inventing one.\n\n"
...
def evaluate_answer(state: InterviewState) -> InterviewState:
    ...
    prompt = (
        ...
        f"Grade this answer against the actual code above.\n\n"
        f"IMPORTANT — do this verification FIRST, before scoring: check whether "
        f"the failure mode, race condition, or edge case implied by the question "
        f"is actually technically real given how this exact code is written. "
        f"Remember JavaScript is single-threaded — code doesn't execute "
        f"concurrently unless real async callbacks/timers/promises are involved. "
        f"If the candidate correctly argues that the implied problem does NOT "
        f"apply here (e.g. because the code is synchronous, or a value is "
        f"recalculated fresh each render), and their reasoning is technically "
        f"sound, score this HIGHLY — do not penalize a correct rebuttal of a "
        f"flawed or overstated premise in the question. Only penalize answers "
        f"that are actually wrong about the real code, not answers that "
        f"correctly push back on an invented failure mode.\n\n"
        f"Be strict but fair — reward correct understanding of the real logic, "
        f"penalize vague or generic answers that don't engage with the "
        f"specific code shown.\n\n"
